...
output$ui_export_main <- renderUI({
  ...
  if (val=="btn_export_results_3") {
    return(uiOutput("ui_export2_data"))
  }
  if (val=="btn_export_results_4") {
...
output$ui_export_sidebar_left <- renderUI({
  choices_export <- reactive({
    ...
    cc <- c("Anonymized Data", "Anonymization Report","Imputation data resulting by anonymized data")
    curLabs <- stataLabs()
    if (!is.null(curLabs)) {
  ...
  eval(parse(text=genObserver_menus(pat="btn_export_results_", n=1:4, updateVal="cur_selection_exports")))
